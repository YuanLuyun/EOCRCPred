...
# 计算预测生存函数
predicted_survival_functions_train = rsf.predict_survival_function(X_train)
predicted_survival_functions_test = rsf.predict_survival_function(X_test)
predicted_survival_functions_external = rsf.predict_survival_function(X_external)
...
def bootstrap_confidence_interval(y_true, X, n_iterations, max_time, specific_times):
    ...
    def compute_resample(seed):
        ...
        rsf.fit(X_resampled, y_resampled)
        predicted_survival_functions = rsf.predict_survival_function(X)

        # 计算Brier分数
        brier_scores = calculate_brier_score(y_resampled, predicted_survival_functions, specific_times)
        integrated_brier = integrated_brier_score(y_resampled, predicted_survival_functions, max_time)
...
if not os.path.exists(csv_file) or os.stat(csv_file).st_size == 0:
    ...
    df = pd.DataFrame({"Label": labels, "Result": ["RSF"] + results})
else:
    # 如果文件存在且不为空，读取文件
    ...
    # 写入数据
    if len(df.columns) < 2:
        df[1] = None  # 如果没有第二列，则创建第二列

    df.iloc[0, 1] = 'RSF'  # 在第二列的第一行写入"RSF"
    df.iloc[1:len(labels) + 1, 0] = labels  # 从第一列的第二行开始写入标签
    df.iloc[1:len(results) + 1, 1] = results  # 在第二列的第二行开始写入结果
